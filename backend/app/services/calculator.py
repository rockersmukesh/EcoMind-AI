...
class TransportationInput(BaseModel):
    milesPerWeek: float = Field(..., ge=0, description="Average miles traveled per week")
    vehicleType: Literal["gas", "hybrid", "ev", "public_transit", "walk_bike"] = Field(
        ..., description="Primary mode of transportation"
    )

class DietInput(BaseModel):
    dietType: Literal["vegan", "vegetarian", "pescatarian", "low_meat", "high_meat"] = Field(
        ..., description="Dietary preference or meat consumption level"
    )

class EnergyInput(BaseModel):
    kwhPerMonth: float = Field(..., ge=0, description="Electricity usage in kWh per month")
    source: Literal["grid", "solar", "wind"] = Field(..., description="Primary electricity source")

class ShoppingInput(BaseModel):
    newClothesPerMonth: float = Field(..., ge=0, description="Number of new apparel items purchased per month")
    techUpgradesPerYear: float = Field(..., ge=0, description="Number of tech/electronics upgrades per year")
...
class CarbonCalculator:
    ...
    def calculate_transportation(input_data: TransportationInput) -> float:
        ...
        factor = EMISSION_FACTORS["transportation"].get(input_data.vehicleType, 0.404)
    ...
    def calculate_shopping(input_data: ShoppingInput) -> float:
        clothes_emissions = input_data.newClothesPerMonth * EMISSION_FACTORS["shopping"]["clothes_item"]
        # Convert annual tech upgrades to monthly
        tech_emissions = (input_data.techUpgradesPerYear / 12) * EMISSION_FACTORS["shopping"]["tech_item"]
